window.Invoices.VIEWCLIENTS = Backbone.View.extend({
    ...
    helperSelected: function(id) {
        $('#invoices_clients_tabs #invoices_clients_tabs-list').children().each(function() {
            if($(this).attr('data-id') != 'item-'+id) $(this).removeAttr('data-selected');
        });
        $('#invoices_clients_tabs [data-id^="content-"]').hide();
        $('#invoices_clients_tabs #invoices_clients_tabs-list [data-id="item-'+id+'"]').attr('data-selected', 'selected');
        $('#invoices_clients_tabs [data-id="content-'+id+'"]').show();        
    }
});
